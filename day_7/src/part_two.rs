...
impl Hand {
    ...
    fn get_hand_type(hand: (String, u32)) -> HandType {
        ...
        let jocker_count: u32 = counts.entry('J' as u32).or_insert(0).clone();
        let mut counts: Vec<(&u32, &u32)> = counts.iter().filter(|x| *x.0 != 'J' as u32).collect();
        counts.sort_by(|a, b| b.1.cmp(a.1));
        let mut counts: Vec<u32> = counts.iter().map(|x| *x.1).collect();
        counts.sort_by(|a, b| b.cmp(a));
        if jocker_count == 5 || counts[0] + jocker_count == 5 {
            HandType::FiveOfAKind
        } else if counts[0] + jocker_count == 4 {
            HandType::FourOfAKind
        } else if counts[0] + jocker_count == 3 && counts[1] == 2 {
            HandType::FullHouse
        } else if counts[0] + jocker_count == 3 {
            HandType::ThreeOfAKind
        } else if counts[0] == 2 && counts[1] + jocker_count == 2 {
            HandType::TwoPair
        } else if counts[0] + jocker_count == 2 {
            HandType::OnePair
        } else {
            HandType::HighCard
        }
...
impl PartialOrd for Hand {
    fn partial_cmp(&self, other: &Self) -> Option<std::cmp::Ordering> {
        if self.hand_type == other.hand_type {
            let vals: Vec<char> = vec!['J', '1', '2', '3', '4', '5', '6', '7', '8', '9', 'T', 'Q', 'K', 'A'];
